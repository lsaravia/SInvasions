...
//
// Se optimizó para hacer la salida tipo "pomac" a de hectareas ocupadas a 14 y 26 años y la salida de densidad a 26 años
// Solamente con el modelo con ruido log normal, el modelo full estocastico se eliminó
// en los parametros se debe poner tFinal 26 años.
//
   	...
   	if( argc==2 )
   	{
		...
		if(!ap.pomac)
		{
			...
			if( !datafile )
				...
				if( priPro )
				{
					datafile << "DimX\tDimY\ttFinal\tg\tdj0\tdj1\tdj2\talfa\talfa1\tdMax\tb\tda\tdensIni\tTime\tJuv\tAd\tJuvOcupa\tAdOcupa" << endl;
					priPro = false;
				}
				...
				simplmat <long> &nOcupado,simplmat <long> &NOcupado)
{
    ...
    double time=0,tempJuv=0,tempAd=0,z1,z2,time50=0,cobertura;
    int dTime=0,dTimeAnt=0;
    int jPlus1,iPlus1,iMinus1,jMinus1;
    long juvOcupado=0,adOcupado=0,adOcupado14=0,adOcupado23=0;
	int salida=0;
...
//			nOcupado(dTime) = juvOcupado;
			...
			// Guarda valor de ocupacion a 14 años (1985)
			//
			if ( dTime==14 )
				adOcupado14=adOcupado;
            ...
            {    
				...
				if( priPro )
				{
					pomacfile << "g\tdj0\tdj1\tdj2\talfa\talfa1\tdMax\tb\tda\tdensIni\tAdGrad1\tAdGrad2\tJuvGrad1\tJuvGrad2\tAdOcupado14\tAdOcupado23" << endl;
					priPro = false;
				}
      	...
      	for( i=0; i<p.DimX; i++)
      		for( j=0; j<p.DimY; j++)
			{
				...
				// Regeneracion local
				tempJuv +=  (1-p.alfa-p.alfa1)*p.b*N(i,j);
				...
				// Dispersion de larga distancia
				//
				
				int dis = ranf()*p.dMax;
				double ang = ranf()*2*M_PI;
				int dx = cos( ang ) * dis ;
				int dy = sin( ang ) * dis ;
				tempJuv += p.alfa1*p.b*N(i+dx,j+dy);
